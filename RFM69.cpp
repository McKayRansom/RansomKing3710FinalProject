...
{
  // Serial.print("Using Select: ");
  // Serial.print(_slaveSelectPin);
  // Serial.print(" and Interrupt: ");
...
//#endif
  ...
  digitalWrite(GPIOE, _slaveSelectPin, 1);
  pinMode(GPIOE, _slaveSelectPin, OUTPUT);
  SPI.begin();
  ...
  if (millis()-start >= timeout)
    return false;
  _inISR = false;
  //attachInterrupt(_interruptNum, RFM69::isr0);//, RISING); //TODO
	// ************** attach interrupt
	//set pin as input
	pinMode(GPIOE, _interruptPin, INPUT);
	
	// enable SYSCFG clock
	RCC->APB2ENR |= RCC_APB2ENR_SYSCFGEN;
	
	//Select PE.10 as the trigger source of EXTI 10
	SYSCFG->EXTICR[2] &= ~SYSCFG_EXTICR3_EXTI10;
	SYSCFG->EXTICR[2] |= SYSCFG_EXTICR3_EXTI10_PE;
	SYSCFG->EXTICR[2] &= ~(0x000F);
	
	//Enable rising edge trigger
	// RSTR is Rising trigger selection register
	// 0 = disabled, 1 = enabled
	EXTI->RTSR1 |= EXTI_RTSR1_RT10;
	
	// Disable falling edge trigger for EXT 10
	// Falling trigger selection register (FSTR)
	// 0 = disabled, 1 = enabled
	EXTI->FTSR1 &= ~EXTI_FTSR1_FT10;
	//Enable EXTI 10 interrupt
	EXTI->IMR1 |= EXTI_IMR1_IM10;
	
	//Set EXTI 10 priority to 1
	NVIC_SetPriority(EXTI15_10_IRQn, 1);
	
	//Enable EXTI 10 interrupt
	NVIC_EnableIRQ(EXTI15_10_IRQn);
	
	//*************** end attach interrupt
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
  ...
  while (digitalRead(GPIOE, _interruptPin) == 0 && millis() - txStart < RF69_TX_LIMIT_MS); // wait for DIO0 to turn HIGH signalling transmission finish
...
void RFM69::writeReg(uint8_t addr, uint8_t value)
{
  ...
  unselect();
	/*select();
  SPI.transfer((value << 8) + (addr | 0x80));
  unselect();*/
}
...
#endif
	*/
  digitalWrite(GPIOE, _slaveSelectPin, 0);
}
