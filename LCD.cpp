...
//enables GPIO clocks and configures GPIO pins as alternate function 11 (LCD)
void LCD_PIN_Init(void){
    ...
    //enable clock to GPIO A,B,C, and D    
    RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN | RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOCEN | RCC_AHB2ENR_GPIODEN;

    //enable all pins as AF 11 (0xB)

    //GPIOA: 6, 7, 8, 9, 10, 15
    //GPIOB: 0, 1, 4, 5, 8, 9, 12, 13, 14, 15
    //GPIOC: 3, 4, 5, 6, 7, 8
    //GPIOD: 8, 9, 10, 11, 12, 13, 14, 15
		
		//set MODER for all pins to Alternate Function b10 (2)
    GPIOA->MODER &= 0xBFEAAFFF;
		GPIOA->MODER |= 0x802AA000;
			
		GPIOB->MODER &= 0xAAFBAAFA;
...
//Displays the first six letters of last name by populating LCD RAM directly
void LCD_DisplayName(void){
  ...
  while ((LCD->SR & LCD_SR_UDR) != 0); // Wait for Update Display Request Bit
    ...
    //K 0x9841   seg F,G,K,N,E
    //I 0x0014   seg A,J,P,D
    //N 0x5a09   seg E,F,H,N,C,B
	
    //R 0xFC01   seg A,B,M,G,N,F,E
    //A 0xFE00   seg E,F,A,B,G,M,C
    //N 0x5a09   seg E,F,H,N,C,B
    //0-1_1000
    //1-
    //2-1_0000_1000
    //3-
    //4-100_0000_0000_0000_0000_0000
    //5-
    //6-1_0000
    //7-
		LCD->RAM[0] |= 1U << 3 | 1U << 4 | 1U << 15 | 1U << 28;
		LCD->RAM[1] |= 0;
		LCD->RAM[2] |= 1U << 3 | 1U << 12 | 1U << 6| 1U << 14| 1U << 29;
		LCD->RAM[3] |= 0;
		LCD->RAM[4] |= 1U << 22 | 1U << 6;
		LCD->RAM[5] |= 0;
		LCD->RAM[6] |= 1U << 4| 1U << 12| 1U << 14| 1U << 15;
		LCD->RAM[7] |= 0;
...
void LCD_Configure(void){
	//1. Configure BIAS[1:0] bits of register LCD_CR and set the bias to 1/3 (10)
    //bias[1] = 1
    LCD->CR |= LCD_CR_BIAS_1;
    //bias[0] = 0
    LCD->CR &= ~LCD_CR_BIAS_0;

    //2. Configure DUTY[2:0] bits of LCD_CR and set the duty to 1/4 (011)
    ...
    //4. Configure PON[2:0] bits of LCD_FCR and set the pulse on period to 111, i.e., 7/ck_ps.
    LCD->FCR |= LCD_FCR_PON;

    //5. Disable the MUX_SEG segment of LCD_CR
    LCD->CR &= ~LCD_CR_MUX_SEG;

    //6. Select internal voltage as LCD (clear VSEL)
    LCD->CR &= ~LCD_CR_VSEL;
    ...
    //10. Wait until the LCD booster is ready by checking the RDY bit LCD_SR
...
//sets up the LCD_RAM and displays the input string on the LCD
